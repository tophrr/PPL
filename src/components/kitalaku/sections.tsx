      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_340px]">
        <GlassPanel className="relative overflow-hidden border-[rgba(124,58,237,0.18)] bg-[#17172b] p-6 text-[var(--slate-900)] shadow-[var(--shadow-premium)] md:p-8">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-8 text-[var(--slate-700)] md:text-base">
              Dari brief sampai approval akhir, halaman ini dirancang untuk mengurangi gesekan kerja tim dan membuat prioritas editorial lebih cepat terbaca.
            </p>
        ...
        <GlassPanel className="p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slate-700)]">Today Focus</p>
              ...
              <div key={item.title} className="rounded-2xl border border-[rgba(219,227,238,0.88)] bg-white/76 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <Link href={item.href} className="mt-3 inline-flex text-xs font-semibold text-[var(--slate-900)]">
                      Open module
                    </Link>
                  </div>
                </div>
    ...
    <div id="planner" className="space-y-5">
      <GlassPanel className="relative overflow-hidden border-[rgba(124,58,237,0.14)] bg-[linear-gradient(145deg,rgba(255,255,255,0.96),rgba(245,247,255,0.9))] p-6 text-[var(--slate-900)] shadow-[var(--shadow-premium)] md:p-8">
        ...
        <div className="relative z-10">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[var(--slate-700)]">AI PLANNER</p>
              <h2 className="font-display mt-4 text-5xl leading-[0.98] text-[var(--slate-900)] md:text-6xl">Buat draft konten baru.</h2>
              <p className="mt-4 max-w-3xl text-base leading-8 text-[var(--slate-600)]">Isi brief singkat, pilih tone, lalu generate caption yang bisa langsung diedit sebelum dikirim ke review.</p>
            </div>
          </div>

          <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
            <div>
              <label htmlFor="planner-brief" className="text-sm font-medium text-[var(--slate-900)]">
      ...
      <GlassPanel className="p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-display text-4xl leading-[1.02] text-[var(--slate-900)]">Arah konten yang lebih matang, bukan sekadar draft otomatis.</h2>
            <p className="mt-2 text-sm leading-7 text-[var(--slate-500)]">Hasil AI di bawah ini sudah disusun seperti workspace nyata: ada topic scoring, manual edit, dan caption draft yang siap dipilih tim.</p>
          </div>
        </div>
      </GlassPanel>
          ...
          {captionDrafts.map((draft) => (
            <article key={draft.platform} className="rounded-2xl border border-[var(--slate-150)] bg-white/80 p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <span className="inline-flex w-fit rounded-full bg-[var(--purple-soft)] px-4 py-2 text-sm font-semibold text-[var(--slate-900)]">{draft.platform}</span>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {draft.tags.map((tag) => (
                  <span key={tag} className="rounded-lg bg-[rgba(124,58,237,0.08)] px-3 py-2 text-sm font-medium text-[var(--slate-900)]">
                    {tag}
                  </span>
                ))}
              </div>
    ...
    <div id="calendar" className="space-y-5">
      <GlassPanel className="p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="font-display text-4xl leading-[1] text-[var(--slate-900)]">Scheduler Board</h2>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button className="rounded-xl bg-[linear-gradient(135deg,#8b5cf6,#7c3aed)] px-4 py-2.5 text-sm font-semibold text-[var(--slate-900)]">+ New Content</button>
          </div>
        </div>
      </GlassPanel>
            ...
            <div className="mt-4 space-y-4">
              {column.cards.map((card) => (
                <article key={card.title} className="rounded-2xl border border-[var(--slate-150)] bg-white/80 p-3">
                  ...
                  <span className="mt-2 inline-flex rounded-full bg-[rgba(124,58,237,0.08)] px-2 py-1 text-xs font-medium text-[var(--slate-900)]">{card.brand}</span>
      ...
      <GlassPanel id="scheduler-calendar" className="p-5">
        <div className="flex flex-col gap-4 border-b border-[var(--slate-150)] pb-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
            ...
            <span className="rounded-full border border-[rgba(124,58,237,0.2)] bg-[rgba(124,58,237,0.09)] px-3 py-1 text-[var(--slate-900)]">Review</span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {day.day === 16 && day.inMonth ? <span className="rounded-full bg-[rgba(124,58,237,0.12)] px-2 py-1 text-[10px] font-semibold text-[var(--slate-900)]">Today</span> : null}
                    </div>
  ...
  return (
    <div id="subscription" className="space-y-5">
      <GlassPanel className="relative overflow-hidden p-6 md:p-7">
        ...
        <div className="relative z-10">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slate-700)]">Subscription Center</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-[var(--slate-900)]">Workspace Plan & Billing</h2>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--slate-500)]">
            UI design untuk memantau paket langganan, kuota AI, dan siklus pembayaran. Data siap dihubungkan ke Convex untuk status plan, usage, invoice, dan riwayat upgrade.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {["Convex: subscriptions", "Convex: usage_events", "Convex: invoices"].map((item) => (
              <span key={item} className="rounded-full border border-[rgba(124,58,237,0.15)] bg-[rgba(124,58,237,0.08)] px-3 py-1.5 text-xs font-semibold text-[var(--slate-900)]">
                {item}
              </span>
            ))}
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <GlassPanel className="p-5 lg:col-span-2">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[var(--slate-150)] pb-4">
            <div>
              ...
              <p className="mt-1 text-3xl font-semibold tracking-tight text-[var(--slate-900)]">Pro Team</p>
              <p className="mt-2 text-sm text-[var(--slate-500)]">Active until 30 Apr 2026 • Auto-renew enabled</p>
            </div>
            <span className="rounded-full bg-[rgba(16,185,129,0.12)] px-3 py-1 text-xs font-semibold text-[var(--emerald-strong)]">Healthy</span>
          </div>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={label} className="rounded-2xl border border-[var(--slate-150)] bg-white/80 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--slate-500)]">{label}</p>
          ...
          <div className="mt-5 rounded-2xl border border-[var(--slate-150)] bg-white/80 p-4">
            ...
            <p className="mt-2 text-xs text-[var(--slate-500)]">Projected to reach ~89% usage before renewal date.</p>
          </div>
        </GlassPanel>
        ...
        <GlassPanel className="p-5">
          ...
          <div className="mt-4 space-y-3">
            <button className="w-full rounded-xl bg-[linear-gradient(135deg,#8b5cf6,#7c3aed)] px-4 py-3 text-sm font-semibold text-[var(--slate-900)]">Upgrade Plan</button>
            <button className="w-full rounded-xl border border-[var(--slate-150)] bg-white/85 px-4 py-3 text-sm font-semibold text-[var(--slate-700)]">Manage Seats</button>
            <button className="w-full rounded-xl border border-[var(--slate-150)] bg-white/85 px-4 py-3 text-sm font-semibold text-[var(--slate-700)]">Download Last Invoice</button>
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-[rgba(245,158,11,0.2)] bg-[rgba(245,158,11,0.08)] p-4">
            ...
            <p className="mt-1 text-xs leading-6 text-[var(--slate-600)]">
              Subscription renews in 14 days. Convex trigger dapat dipakai untuk kirim notifikasi in-app + email H-7 dan H-1.
            </p>
          </div>
        </GlassPanel>
      </div>
      ...
      <GlassPanel className="p-5">
        <div className="flex flex-col gap-4 border-b border-[var(--slate-150)] pb-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--slate-500)]">Mock data untuk desain tabel invoice (source nantinya dari Convex query).</p>
          </div>
          ...
          <table className="min-w-[760px] w-full text-left text-sm">
            ...
            <tbody>
              {[
                ...
                <tr key={row[0]} className="border-t border-[var(--slate-150)] text-[var(--slate-700)]">
                  <td className="px-3 py-3 font-semibold">{row[0]}</td>
                  <td className="px-3 py-3">{row[1]}</td>
                  <td className="px-3 py-3">{row[2]}</td>
                  <td className="px-3 py-3">
                    <span className="rounded-full bg-[rgba(16,185,129,0.12)] px-2.5 py-1 text-xs font-semibold text-[var(--emerald-strong)]">{row[3]}</span>
                  </td>
    ...
    <div id="analytics" className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
      <GlassPanel className="p-5">
        <div className="rounded-[20px] bg-[linear-gradient(135deg,#16a34a_0%,#22c55e_100%)] px-5 py-4 text-[var(--slate-900)]">
          ...
          <p className="mt-2 text-sm text-[var(--slate-900)]">Agency-client workflow dengan keputusan yang lebih cepat dan tetap terdokumentasi.</p>
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
      ...
      <GlassPanel className="p-5">
        <div className="rounded-[20px] bg-[linear-gradient(135deg,#7c3aed_0%,#a855f7_100%)] px-5 py-4 text-[var(--slate-900)]">
          ...
          <p className="mt-2 text-sm text-[var(--slate-900)]">Performance metrics yang lebih siap dibaca untuk evaluasi campaign dan next move.</p>
        </div>
