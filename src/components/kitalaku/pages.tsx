      ...
      <div className="flex items-center gap-3">
        <Unauthenticated>
          <SignInButton mode="modal">
            <button className="rounded-lg border border-(--slate-150) bg-white/80 px-4 py-2 text-sm font-semibold text-(--slate-700) hover:bg-white transition-all">
              Login
            </button>
          </SignInButton>
        </Unauthenticated>
...
function LandingHero() {
  return (
    <div className="grid gap-6 xl:grid-cols-[1.12fr_0.88fr]">
      <section className="relative overflow-hidden rounded-[36px] bg-[#FFFFFF] p-8 text-(--slate-900) shadow-(--shadow-premium) md:p-10">
        ...
        <div className="relative z-10">
          <p className="text-xs font-semibold uppercase tracking-[0.34em] text-(--slate-700)">
            AI CONTENT WORKSPACE
          </p>
          <h1 className="font-display mt-4 max-w-4xl text-3xl leading-[1.06] text-(--slate-900) md:text-6xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link
              href="/dashboard"
              className="rounded-xl border border-(--slate-200) bg-white/50 px-8 py-4 text-lg font-semibold text-(--slate-700) hover:bg-white transition-all"
            >
              Jelajahi dashboard
            </Link>
          </div>
      ...
      <Authenticated>
        <GlassPanel className="p-8 flex flex-col justify-center items-center text-center">
          <div className="w-16 h-16 rounded-full bg-(--purple-soft) flex items-center justify-center mb-6">
            <UserButton appearance={{ elements: { userButtonAvatarBox: 'w-16 h-16' } }} />
          </div>
          <h2 className="font-display text-3xl leading-[1.08] text-(--slate-900) md:text-4xl">
...
export function LandingPageDesign() {
  return (
    <div className="premium-background min-h-screen px-4 py-6 md:px-6 lg:px-10">
      <div className="mx-auto max-w-270 space-y-8">
        <LandingNav />
        <LandingHero />
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {[
            ...
            <GlassPanel key={title} className="p-5">
              <p className="font-display text-2xl leading-none text-(--slate-900)">{title}</p>
        ...
        <Unauthenticated>
          <GlassPanel className="flex flex-col items-center justify-between gap-4 p-6 text-center md:flex-row md:text-left">
            <div>
              <p className="font-display text-3xl leading-[0.98] text-(--slate-900)">
...
export function PlannerPageDesign() {
  return (
    <AppShell active="AI Planner">
      <Suspense
        fallback={
          <div className="p-8 text-center text-sm text-(--slate-500)">Memuat Perencana...</div>
        }
      >
        <PlannerSection />
      </Suspense>
...
export function LoginPageDesign({
  ...
  return (
    <div className="premium-background relative min-h-screen overflow-hidden px-4 py-8 md:px-6">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-295 items-center">
        <div className="grid w-full gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          <section className="relative hidden overflow-hidden rounded-[32px] bg-[#FFFFFF] p-8 text-(--slate-900) shadow-(--shadow-elevated) lg:flex lg:flex-col lg:justify-between">
            ...
            <div className="relative z-10">
              ...
              <h1 className="font-display mt-4 text-5xl leading-[0.98] text-(--slate-900)">
                Masuk ke workspace
              </h1>
            </div>
          </section>
          ...
          <div className="mx-auto w-full max-w-155">
            <div className="mb-4 flex flex-wrap gap-2 lg:hidden">
              {['Role-based access', 'Persistent session', 'Premium workspace'].map((pill) => (
                <span
                  key={pill}
                  className="rounded-full border border-[rgba(219,227,238,0.88)] bg-white/80 px-3 py-1.5 text-xs font-semibold text-(--slate-600)"
                >
                  {pill}
                </span>
              ))}
