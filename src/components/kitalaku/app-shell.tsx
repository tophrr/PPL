...
export function AppShell({
  ...
  const pageDescriptions: Record<string, string> = {
    ...
    Subscription: "Track plan limits, billing cycle, and Convex-backed usage status from one place.",
  ...
  return (
    <div className="min-h-screen md:pl-[292px]">
      ...
      <aside
        className="hidden rounded-[32px] border border-[rgba(219,227,238,0.88)] bg-[linear-gradient(180deg,rgba(255,255,255,0.88),rgba(255,255,255,0.72))] shadow-[0_24px_48px_rgba(30,41,59,0.14)] backdrop-blur-2xl md:fixed md:bottom-4 md:left-4 md:top-4 md:flex md:w-[260px] md:flex-col md:justify-between md:overflow-y-auto"
      >
        <div className="px-5 py-5">
          <GlassPanel className="p-5">
            <p className="font-display text-3xl leading-[0.95] text-[var(--slate-900)]">Kitalaku.in</p>
            <p className="mt-1 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slate-700)]">Creative OS</p>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-stretch justify-end overflow-hidden p-2 md:p-3">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--slate-700)]">Inbox</p>
                  <h2 id="notification-title" className="mt-1 text-xl font-semibold tracking-tight text-[var(--slate-900)]">
                    Notifications
                  </h2>
                </div>
                <button
              ...
              <div className="mt-4 space-y-3">
                {[
                  { title: "Approval updated", detail: "Blog Post - Marketing Tips changed to Approved.", time: "5m ago" },
                  { title: "New review requested", detail: "Instagram Carousel - April Promo is waiting for your review.", time: "18m ago" },
                  { title: "Schedule adjusted", detail: "Weekly newsletter moved to Friday 10:00 AM.", time: "42m ago" },
                  { title: "AI draft completed", detail: "3 caption drafts for Product Launch are ready.", time: "1h ago" },
                ].map((item) => (
                  <div
                    key={item.title}
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-[var(--slate-500)]">{item.detail}</p>
                      </div>
